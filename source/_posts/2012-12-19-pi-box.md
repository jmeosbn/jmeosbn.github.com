...
There's currently no build of the Dropbox sync client for ARM devices; while there is source available, it's not suitable for automatic, unattended sync on a Raspberry Pi that may not have any windowing system installed.
...
To run dbox periodically, download[^dl] the [example script], make it executable, then replace the example keys and path settings with your own.
...
All output is written to a log file, so it's normal not to see any output while running; change the script so `LOGFILE = STDOUT` if you want to see some basic info while testing.  To show the full syncing progress and other debug info, set the level of the [logger class] by changing the script so it reads `LOGGER.level = Logger::DEBUG`.
...
```sh
	...
	if ! pgrep -f dboxsync; then
	  sudo -u xbian /home/xbian/dboxsync.rb &
	fi
```
...
Download [source archive](https://www.dropbox.com/download?dl=packages/nautilus-dropbox-1.4.0.tar.bz2) and build as usual, more details are [here](https://www.dropbox.com/help/247).
